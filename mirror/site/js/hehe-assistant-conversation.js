...
(function(){
  ...
  var DRAFT_TTL_MS=8*60*60*1000;
  var PRIVATE_DRAFT_FIELDS=/^(?:password|pwd|passcode|token|accessToken|refreshToken|secret|authorization|cookie|apiKey|api_key|idCard|identityCard|credential|phone|mobile|contact)$/i;
  ...
  function bootFailure(){
    ...
    console.error('[HHagent] conversation module dependencies were not ready.');
  }

  function boot(){
    if(!window.HeheV4||!window.HeheForms){
      bootAttempts+=1;
    ...
    function startOfWeek(d){
      ...
      var weekday=base.getDay()||7;
      base.setDate(base.getDate()-weekday+1);
      return base;
    }
    function dateValue(text){
      var now=new Date();now.setHours(0,0,0,0);
      var match=text.match(/(20\d{2})[-年/](\d{1,2})[-月/](\d{1,2})/);
      if(match){
        var exact=validDateParts(Number(match[1]),Number(match[2]),Number(match[3]));
        return exact?localIso(exact):'';
      }
      match=text.match(/(\d{1,2})月(\d{1,2})[日号]?/);
      if(match){
        var month=Number(match[1]),day=Number(match[2]);
        var candidate=validDateParts(now.getFullYear(),month,day);
        if(!candidate)return '';
        if(candidate<now)candidate=validDateParts(now.getFullYear()+1,month,day);
        return candidate?localIso(candidate):'';
      }
      var d=new Date(now);
      if(/后天/.test(text))d.setDate(d.getDate()+2);
        ...
        var targetMap={'一':1,'二':2,'三':3,'四':4,'五':5,'六':6,'日':7,'天':7};
        var target=targetMap[match[2]];
        if(match[1]==='本'||match[1]==='下'){
          d=startOfWeek(now);
          if(match[1]==='下')d.setDate(d.getDate()+7);
          d.setDate(d.getDate()+target-1);
        }else{
          var current=now.getDay()||7;
    ...
    function looksLikeQuestion(text){
      return /[?？]$/.test(text)||/^(?:请问|我想问|想问|有没有|是否|能否|能不能|怎么|为什么|哪里|哪些|多少|什么时候|可以吗|行不行)/.test(text);
    }
    function parse(p,text,allowFallback){
      var values=p.values,fields=rows(p.config,values),changed=[];
      function set(key,value){if(value&&fields.some(function(f){return f.key===key;})){values[key]=value;if(!changed.includes(key))changed.push(key);}}
      ...
      fields.forEach(function(f){
        ...
        var valueMatch=text.match(new RegExp(label+'[：:]\\s*([^；;\\n]+)'));
        if(valueMatch&&f.type!=='file')set(f.key,valueMatch[1].trim());
      });
      if(allowFallback!==false&&!changed.length&&!looksLikeQuestion(text)){
        var next=missing(p)[0];
        if(next&&['text','textarea','tel','number'].includes(next.type)&&text.length<=200)set(next.key,text);
      }
      return changed;
    }
    function cleanDraftModel(values){
      var clean={};
      Object.keys(values||{}).forEach(function(key){
        if(PRIVATE_DRAFT_FIELDS.test(key))return;
        var value=values[key];
        if(value&&typeof value==='object'&&!Array.isArray(value))clean[key]=Object.assign({},value,{live:false});
    ...
    function persist(p){
      try{
        sessionStorage.setItem('hehe-ui-v2:draft:'+p.config.key,JSON.stringify({
          model:cleanDraftModel(p.values),
          step:0,
    ...
    function readDraft(key){
      try{
        var storageKey='hehe-ui-v2:draft:'+key;
        var saved=JSON.parse(sessionStorage.getItem(storageKey)||'null');
        if(saved&&saved.expiresAt&&saved.expiresAt<Date.now()){sessionStorage.removeItem(storageKey);return null;}
        return saved;
      }catch(e){return null;}
    }
    ...
    function showIntake(p,initial){
      ...
      if(filled.length){
        var detail=el('details','hh-collected');
        detail.appendChild(el('summary','','已整理 '+filled.length+' 项信息'));
        var list=el('dl','hh-review');
        filled.forEach(function(f){
          var r=el('div');
          r.append(el('dt','',f.label),el('dd','',typeof p.values[f.key]==='object'?p.values[f.key].name:String(p.values[f.key])));
          list.appendChild(r);
        });
        detail.appendChild(list);c.appendChild(detail);
      }
      ...
      actions.appendChild(button(remaining.length?'打开表单补充':'核对并继续',function(){
        ...
        var next=Object.assign({},p.config,{expand:true,collected:p.values,reviewOnly:!missing(p).length,prompt:'核对'+p.config.title+'信息'});
        retire(p);pending=null;p.open(next);
      },!remaining.length));
      actions.appendChild(button('先不办理',function(){
        if(p!==pending)return;
        retire(p);pending=null;
        say('好的，非敏感信息已留作本次会话草稿；联系人和手机号不会保存。您可以继续问我其他问题。');
      }));
      c.appendChild(actions);p.card=c;ui.botCard(c,260);
    }
    function prepare(config,open){
      retire(pending);
      var saved=readDraft(config.key);
      ...
      if(lastInput)parse(pending,lastInput,false);
      persist(pending);showIntake(pending,true);
    }
    function detect(text){
      ...
      if(/进度|申请.*(?:到哪|哪一步)|办理到|待办|撤销|撤回/.test(text))return ['progress'];
    ...
    function send(text){
      ...
      if(!text)return;
      window.HeheIO.ensureOpen();
      input.value='';
      input.dispatchEvent(new Event('input',{bubbles:true}));

      if(pending&&/^(取消|先不办|不办了|结束办理)/.test(text)){
        retire(pending);pending=null;ui.userSay(text);
        say('好的，我们先停在这里。您还想了解什么？');
        return;
      }

      var target=detect(text);
      var newTopic=/^(帮我找|我要查|换个|转人工|查进度|我想预约|我要预约|问一下政策|了解政策)|申请.*(?:需要准备|需要什么材料)/.test(text);

      if(pending&&!newTopic){
        ui.userSay(text);
        var changed=parse(pending,text,false);
        if(!changed.length&&looksLikeQuestion(text)){
          say('当前正在准备“'+pending.config.title+'”。这句话看起来是一个新问题，我没有把它写入表单。您可以先说“先不办理”，再继续提问。');
          return;
        }
        if(!changed.length)parse(pending,text,true);
        persist(pending);showIntake(pending,false);
        return;
      }
      ...
      if(target){
        if(target[0]==='resource')window.HeheForms.resource(/GPU|算力/i.test(text)?'GPU 训练算力':'数据资源');
        else ui.runPrompt(text,target[0],target[0]==='zero'?escape(target[1]):target[1]);
      }else{
        ui.userSay(text);
        say(/你好|您好|hello|hi/i.test(text)
          ?'您好！我是合合。想了解什么，或有什么事需要我帮忙？'
          :'您想找资源、了解办理条件，还是办理一件具体的事？可以直接说，例如“帮我找医疗影像数据”或“我想预约参观”。');
      }
      lastInput='';
    }

    var fullDiscover=scenes.discover,fullCatalog=scenes.catalog,fullPolicy=scenes.policy,fullRecommend=scenes.recommend;
    scenes.firstvisit=function(){
      ui.userSay('这个平台能帮我什么？');
      say('我是合合，陪您使用数智北京创新中心的服务。\n您可以直接说想做什么：找数据和算力、了解政策与材料，或预约参观、跟踪申请。我会先帮您理清需求，必要时再打开办理页面。');
    };
    scenes.discover=function(kind){
      kind=kind||'医疗';
      ui.userSay('帮我找'+kind+'资源');
      say('先为您看这两个方向。您也可以继续告诉我具体用途，我再帮您缩小范围。');
      var c=ui.card('',null);c.classList.add('hh-agent-results');
      var data=kind==='算力'
        ?[['GPU 训练型服务器','适合模型训练，申请时需说明规模和周期'],['模型评测服务','用于模型质量与安全评测']]
        :kind==='客流'
          ?[['城市交通流量数据集','用于客流预测与交通研究'],['时空预测算法组件','配合交通数据进行模型验证']]
          :[['脱敏医学影像数据集','用于医学影像算法研发与验证'],['图像预处理组件','用于影像整理、清洗与模型前处理']];
      data.forEach(function(d){
        var item=el('div','hh-agent-resource');
        item.append(el('strong','',d[0]),el('p','',d[1]),button('帮我申请',function(){window.HeheForms.resource(d[0]);}));
        c.appendChild(item);
      });
      c.appendChild(el('small','hh-field-hint','预置资源示例，实际供给与授权条件以平台为准。'));
      c.appendChild(button('查看更多匹配资源',function(){fullDiscover(kind);}));
      ui.botCard(c,260);
    };
    scenes.policy=function(){
      topic='policy';
      ui.userSay('申请资源需要准备什么？');
      var compute=/算力|GPU|云网算/i.test(lastInput),data=/数据/.test(lastInput);
      say(compute
        ?'可以，先为您整理算力申请的材料要点：\n1. 项目名称与训练、推理等具体用途；\n2. GPU / 存储等资源规格、数量和使用周期；\n3. 申请机构、联系人及相关方案材料。\n这是原型材料清单，正式要求以平台办理指南为准。'
        :data
          ?'数据资源申请可以先准备：\n1. 项目名称、数据用途和所需字段范围；\n2. 使用周期、数据安全措施与相关方案；\n3. 申请机构和负责人的联系方式。\n这是原型材料清单，实际开放范围与授权条件以平台为准。'
          :'申请前，需要明确项目用途、资源规模和使用周期，并准备机构、联系人与相关方案材料。\n您想了解数据申请，还是算力申请？');
      var c=ui.card('',null);c.classList.add('hh-agent-inline');
      if(compute||data)c.appendChild(button('帮我准备'+(compute?'算力':'数据')+'申请',function(){window.HeheForms.resource(compute?'GPU 训练算力':'数据资源');}));
      c.appendChild(button('查看政策与材料示例',fullPolicy));
      ui.botCard(c,260);
    };
    scenes.catalog=function(){
      if(/教委|医疗|交通|部门/.test(lastInput)){fullCatalog();return;}
      ui.userSay('查询数据目录');
      say('可以按部门、行业或具体资源查询。比如“市教委有哪些数据集”或“医疗领域有哪些开放数据”。');
      var c=ui.card('',null);c.classList.add('hh-agent-inline');
      c.appendChild(button('展开目录统计示例',fullCatalog));
      ui.botCard(c,260);
    };
    scenes.recommend=function(){
      ui.userSay('推荐适合我的服务');
      say('您目前主要做什么方向，最需要数据、算力，还是合作场景？先告诉我需求，推荐会更有针对性。');
      var c=ui.card('',null);c.classList.add('hh-agent-inline');
      c.appendChild(button('调整推荐偏好',fullRecommend));
      ui.botCard(c,260);
    };

    document.addEventListener('click',function(e){
      if(e.target.closest&&e.target.closest('#chatSend')){
        e.preventDefault();e.stopImmediatePropagation();send(input.value);
      }
    },true);
    ...
    window.HeheAgent={
      send:send,
      start:function(text){retire(pending);pending=null;topic='';send(text);},
      prepare:prepare,
      cancel:function(){retire(pending);pending=null;topic='';},
      clearDrafts:function(){
        try{
          Object.keys(sessionStorage).forEach(function(key){
            if(key.indexOf('hehe-ui-v2:draft:')===0)sessionStorage.removeItem(key);
          });
        }catch(error){}
      }
    };
