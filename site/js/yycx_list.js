$(function () {
  ...
  // 1. 全局纯状态
  const state = { appGroupId: 1, pageIndex: 1 }
  ...
  function updateYycxTabCount(totalCount) {
    var $tabLink = $('.yysc_title a[href="/yyscww/cgcx"]')
    if ($tabLink.length) {
      $tabLink.text('应用创新(' + (totalCount || 0) + ')')
    }
  }
...
*/
  ...
  function renderDataToPage(records) {
    ...
    records.forEach((record) => {
      ...
      // 提取字段
      var createTime = record.createTime.substring(0, 10)
      ...
      htmlContent += `
            		<li>
                        ...
                        <div class="info">
                            ...
                            <div class="desc">${record.appGroupName}</div>
                        </div>
                        <div class="info">
                            <div class="bt">上架时间：</div>
                            <div class="desc">${createTime}</div>
                        </div>
                        <div class="btn"><a href="/yyscww/cgcx/xqy?id=${record.id}" target="_blank">了解更多</a></div>
                    </li>
        `
    })
